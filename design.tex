...
%To reduce device-side complexity of design, we use a reverse fuzzy extractor (RFE) that moves the complex error correction process from the device to the server \cite{van2012reverse}. 
...
During the enrollment phase, the server reads out the power up values of SRAM cells through a one-time interface, runs the \texttt{FE.Gen} to obtain the helper data (for subsequent reconstructions), and stores the secret key $\mathbf{s}$ in the server \texttt{DB}.
...
With $\mathbf{s}$ securely transferred to the server side in PUF enrollment phase, the computational expensive operations such as discrete Gaussian sampling ($\mathbf{e}$) can happen on the server side rather than the PUF side.
...
During each authentication, the error correction uses the code-offset construction with a concatenated code. 
If the HD between $\mathbf{s}$ and $\mathbf{s}_R$ is within the error-decoding capability of the ECC decoder, the device reconstructs the correct secret key $\mathbf{s}$. 
