...
If the HD between $\mathbf{s}$ and $\mathbf{s}_R$ is within the error-decoding capability of the error correction code (ECC) decoder, the decoder succeeds in reconstructs the correct secret key  $\mathbf{s}$ (\texttt{FE.Rec}). 
