...
During the enrollment phase, the POK bits, based on the power-up values of SRAM cells, are read out through a one-time interface and sent to the server. The fuzzy extractor generates helper data 
...
Each blocks shares the same LFSR seed, samples a string $\mathbf{r}$ with length $\text{Len}$, a binary matrix $\mathbf{X}$ of dimension $m\times \text{Len}$, and calculates a vector $\mathbf{b}^\prime$ of length $\text{Len}$, in which $\text{Len}$ is the length of PUF responses required by one authentication.  
...
If the HD between $\mathbf{s}$ and $\mathbf{s}_R$ is within the error-decoding capability of the error correction code (ECC) decoder, the decoder succeeds in reconstructs the correct secret key  $\mathbf{s}$. 
...
The server compares the $\text{Len}$-bit $\mathbf{r}_R$ string with the corresponding $\text{Len}$-bit $\mathbf{r}$ string stored in DB:
