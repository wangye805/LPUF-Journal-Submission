...
\begin{figure}[t!]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.9\linewidth]{./figs/protocol}
%DIFDELCMD < %%%
...
\subsection{\DIFdelbegin \DIFdel{Application Scenario and Reverse Fuzzy Extractor}\DIFdelend \DIFaddbegin \DIFadd{PUF-Based Authentication}\DIFaddend }
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To reconstruct stable POK bits }\DIFdelend \DIFaddbegin \DIFadd{As an example application, we study using the lattice PUF in a string-matching authentication scheme \mbox{%DIFAUXCMD
\cite{suh2007physical}}\hspace{0pt}%DIFAUXCMD
. 
To reconstruct the same secret key from POK bits each time}\DIFaddend , we adopt the fuzzy extractor (FE) \cite{bosch2008efficient} method to achieve a low failure rate. 
...
. As an example application, we study using the lattice PUF in a string-matching authentication scheme \mbox{%DIFAUXCMD
\cite{suh2007physical}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
\DIFadd{During the enrollment phase, the server reads out the power-up values of SRAM cells through a one-time interface, runs the helper data generation block in FE }\texttt{\DIFadd{FE.Gen}} \DIFadd{(for subsequent reconstructions}\DIFaddend ), and \DIFdelbegin \DIFdel{a lattice PUF. The server has a database (}\DIFdelend \DIFaddbegin \DIFadd{stores both secret key $\mathbf{s}$ and helper data $\mathbf{h}$ in the server }\DIFaddend \texttt{DB}\DIFdelbegin \DIFdel{) to store the POK, an RFE reconstruction block (}\texttt{\DIFdel{RFE.Rec}}%DIFAUXCMD
\DIFdel{),
...
%DIFDELCMD < %%%
\DIFdel{Figure \ref{fig:protocol} shows the operations of an end-to-end authentication method using the lattice PUF. During the enrollment phase, }\DIFdelend \DIFaddbegin \DIFadd{.
...
With $\mathbf{s}$ securely transferred to the server side in PUF enrollment phase, }\DIFaddend the \DIFdelbegin \DIFdel{server reads out the POK }\DIFdelend \DIFaddbegin \DIFadd{computationally expensive discrete Gaussian sampling ($\mathbf{e}$) can happen on the server side rather than the PUF side.
Moreover, keeping }\DIFaddend $\mathbf{s}$ \DIFdelbegin \DIFdel{from the SRAM cells through a one-time interface, and stores it in the server }\texttt{\DIFdel{DB}}%DIFAUXCMD
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{on the server side also enables challenge compression via dimension relaxation. 
...
The device generates the PUF response $\mathbf{r}_R$ \DIFaddbegin \DIFadd{by running the PUF function $\mathbf{r}_R = \text{PUF}(ch, \mathbf{s})$ }\DIFaddend and sends it to the server. 
The server compares $\mathbf{r}_R$ with the response $\mathbf{r}$ \DIFdelbegin \DIFdel{generated from the server-side lattice PUF. The device will be }\DIFdelend \DIFaddbegin \DIFadd{stored in DB:
the device is }\DIFaddend accepted if $\text{HD}(\mathbf{r}_R, \mathbf{r})$ is less than a \DIFaddbegin \DIFadd{given }\DIFaddend threshold $th$.
