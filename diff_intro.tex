...
\DIFdelbegin \DIFdel{(}\DIFdelend An NVM-based key storage mechanism in place of the POK is an alternative approach\DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{, but is not able to fulfill the storng PUF definition }\DIFaddend with reduced security against physical key-extraction attacks.
\DIFdelbegin \DIFdel{)
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The core module of the lattice PUF is the LWE decryption function\DIFdelbegin \DIFdel{. It generates a response (plaintext) for each submitted challenge (ciphertext). By itself, this block represents a keyed LWE-based hash function with restricted inputs (which follow the ciphertext distribution). 
The restriction on inputs permits a more compact design compared to other known LWE-based pseudorandom functions, e.g. \mbox{%DIFAUXCMD
\cite{brenner2014fpga}}\hspace{0pt}%DIFAUXCMD
. 
...
The design }\DIFdelend \DIFaddbegin \DIFadd{, which guarantees the ML resistance under a dedicated input distribution. 
The design }\DIFaddend is carefully crafted considering trade-offs between various aspects.
...
\DIFdelbegin \DIFdel{Our reliance on LWE to formally guarantee resistance to ML attacks is novel. 
We highlight a critical difference between our work and recent work  \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga}}\hspace{0pt}%DIFAUXCMD
.
...
In short, the input-output mapping between the PUF of \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga} }\hspace{0pt}%DIFAUXCMD
and the underlying LPN or LWE cryptosystem can be briefly summarized as follows: 
...
%DIFDELCMD < %%%
\DIFdel{Although both constructions utilize lattice-based cryptosystems, they differ fundamentally in the root of their security guarantees.
The fundamental security property that \mbox{%DIFAUXCMD
\cite{fuller2013computational} }\hspace{0pt}%DIFAUXCMD
rely upon is the }\emph{\DIFdel{computational hardness of recovering a private key from a public key in a public-key cryptosystem}}%DIFAUXCMD
\DIFdel{. 
It turns out that, when building strong PUFs using public keys as challenges and private keys as responses, this security is insufficient. The vulnerability is due to the fact that the challenges are by definition publicly known \mbox{%DIFAUXCMD
\cite{fuller2013computational,herder2017trapdoor,jin2017fpga}}\hspace{0pt}%DIFAUXCMD
, and an attacker can use multiple challenges (public keys) to recover the private key. This is only possible because multiple public keys are derived using a fixed (same) source of secret POK bits, embedded in the error term of LPN or LWE. As was shown in \mbox{%DIFAUXCMD
\cite{apon2017efficient}}\hspace{0pt}%DIFAUXCMD
, the fact that multiple CRPs have shared error terms can be easily exploited, which allows a computationally-inexpensive algorithm for solving an LPN or LWE instance. %DIF < thus compromising the hardness of LPN or LWE problems. Thus, by itself \cite{fuller2013computational,herder2017trapdoor,jin2017fpga}, \emph{the resulting PUF does not have resistance against ML modeling attacks}. This vulnerability is fixed in \cite{herder2017trapdoor,jin2017fpga} by introducing a cryptographic hash function to hide the original CRPs. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\emph{\DIFdel{In stark contrast, the proposed lattice PUF derives its security by directly exploiting a distinctly different property of public-key cryptosystems: the theoretically-proven guarantee that their decryption functions are not PAC learnable}}%DIFAUXCMD
\DIFdel{. 
...
%DIFDELCMD < %%%
\DIFdel{Further, the construction of  \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga} }\hspace{0pt}%DIFAUXCMD
is expensive since (1) recovering secrets from helper data requires solving a system of linear equations, and (2) implementing a hash function is costly.  
...
%DIFDELCMD < %%%
\DIFdelend Since a PUF may be deployed in very different applications, it is critical to convert a theoretically sound construction into an efficient physical implementation. 
...
\DIFaddbegin \DIFadd{Though all inspired by the LWE problem, we highlight key differences of our lattice PUF against the strong PUF based on LWE/LPN CFE \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga}}\hspace{0pt}%DIFAUXCMD
, a LWE-based pseudorandom function \mbox{%DIFAUXCMD
\cite{brenner2014fpga}}\hspace{0pt}%DIFAUXCMD
, and the direct implementation of a LWE public-key cryptosystem.
}

\DIFadd{First, compared to the strong PUF based on LWE/LPN CFE \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga}}\hspace{0pt}%DIFAUXCMD
...
\DIFadd{Although both constructions utilize lattice-based cryptosystems, they differ fundamentally in the root of their security guarantees.
The fundamental security property that \mbox{%DIFAUXCMD
\cite{fuller2013computational} }\hspace{0pt}%DIFAUXCMD
rely upon is the }\emph{\DIFadd{computational hardness of recovering a private key from a public key in a public-key cryptosystem}}\DIFadd{. 
...
The vulnerability is due to the fact that the challenges are by definition publicly known \mbox{%DIFAUXCMD
\cite{fuller2013computational,herder2017trapdoor,jin2017fpga}}\hspace{0pt}%DIFAUXCMD
, and an attacker can use multiple challenges (public keys) to recover the private key. 
...
As was shown in \mbox{%DIFAUXCMD
\cite{apon2017efficient}}\hspace{0pt}%DIFAUXCMD
, the fact that multiple CRPs have shared error terms can be easily exploited, which allows a computationally-inexpensive algorithm for solving an LPN or LWE instance. 
}

\emph{\DIFadd{In stark contrast, the proposed lattice PUF derives its security by directly exploiting a distinctly different property of public-key cryptosystems: the theoretically-proven guarantee that their decryption functions are not PAC learnable}}\DIFadd{. 
(As is shown later, this property stems from the semantic security of a cryptosystem coupled with the ease of generating multiple ciphertexts.) 
Lattice PUF does not have the vulnerability mentioned above since the publicly known challenges are ciphertexts and the security of the public-key cryptosystem guarantees that the fixed private key (the POK, in our case) cannot be recovered from ciphertexts.
}

\DIFadd{Further, the construction of  \mbox{%DIFAUXCMD
\cite{herder2017trapdoor,jin2017fpga} }\hspace{0pt}%DIFAUXCMD
is expensive since (1) recovering secrets from helper data requires solving a system of linear equations, and (2) implementing a hash function is costly.  
In contrast, our PUF is lightweight since it implements only the LWE decryption function rather than the expensive encryption function.
}

\DIFadd{Second, our lattice PUF construction can be seen as a keyed LWE-based hash function with restricted inputs (which follow the ciphertext distribution). 
...
In particular, this restriction on inputs permits a more compact design compared to other known LWE-based pseudorandom functions, e.g. \mbox{%DIFAUXCMD
\cite{brenner2014fpga}}\hspace{0pt}%DIFAUXCMD
, which guarantees more than just ML resistance. 
}

\DIFadd{Third, our lattice PUF construction is different from a direct implementation of LWE public-key cryptosystem with key generated from POKs in the following aspects. 
...
\DIFaddend Our lattice PUF construction achieves a CRP space size of $2^{136}$.
